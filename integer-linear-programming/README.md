# Integer linear programming - Street Directionality Problem
## Combinatorial Problem Solving course
This project solves the Street Directionality Problem using an integer linear programming approach. It contains a single C++ source file together with a few scripts to run the solver and validate the generated outputs. Below you can find the instructions required to build and execute the code. The exact commands shown in this file are the same ones used to generate the results available in the [out directory](./out).

## How to build
### On the host machine using the Makefile
You can compile the project on a Linux machine using the [Makefile](./src/Makefile) located in the `src` directory. You only need to modify the `CPLEXDIR` and `CONCERTDIR` variables according to your local CPLEX installation.
...
The preferred option for running this project is to use Docker. This creates a Debian-based environment with CPLEX installed, together with the `sdp` and `sdp-checker` commands used to run the solver and the checker, respectively. One important remark is that since CPLEX requires a license to be used and this project was done using the academic version. Some instances might be too large for the Community Edition, in order to prevent this, the CLPEX installation is done by using an installer provided as build argument for the Docker image. To do this, provide the name of the installer file using the `--build-arg` flag and the `CPLEX_INSTALLER` name, the file must be **in the same directory** as the Dockerfile. E.g.:

```bash
docker build --build-arg CPLEX_INSTALLER=my-academic-installer.bin -t ilp-sdp .
```
...
```bash
docker run -v ./input:/app/input -v ./out:/app/out -v ./other:/app/other -ti ilp-sdp /bin/bash
```
